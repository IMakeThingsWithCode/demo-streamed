...
export class xdofool {
    ...
    static convert(config: {
        ...
        exec(`ffmpeg -framerate ${config.framerate} -i ${config.recordDir}${config.stream.name}%0${config.framecounterDigits}d.tga -c:v libx264 -pix_fmt yuv420p ${config.recordDir}${config.stream.name}.mp4`)
    }
    static screenshot(config: {
        ...
        exec(`scrot -Z 0 -u -q 100 ${config.recordDir}${config.stream.name}${'0'.toString().repeat(config.framecounterDigits - config.frameNumber.toString().length)}${config.frameNumber}.tga`)
    }
}